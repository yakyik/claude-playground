...
func init() {
	provider.Register("gcp", func() provider.Provider { return New() })
}

// Provider manages GCP infrastructure via Terraform.
...
func New() *Provider {
	return &Provider{
		tfDir: filepath.Join("providers", "gcp"),
	}
}
...
func (p *Provider) Status(ctx context.Context) (*provider.Status, error) {
	...
	out, err := cmd.Output()
	if err != nil {
		return &provider.Status{Running: false, Provider: "gcp"}, nil
	}
...
func (p *Provider) SSHConfig(ctx context.Context) (string, error) {
	...
	if !st.Running || st.Address == "" {
		return "", fmt.Errorf("GCP instance is not running")
	}
